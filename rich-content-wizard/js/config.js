...
urlMappings: [{
    ...
    performValidationChecks: function(fullHtmlCode, tempDoc, monacoEditorInstance, errors) {
        ...
        while ((match = tagRegexForValidation.exec(fullHtmlCode)) !== null) {
            ...
            if (isClosingTag) {
                ...
                if (this.deprecatedTags.has(tagName)) errors.push({ message: `<strong>Deprecated Tag:</strong> The &lt;${tagName}&gt; tag is deprecated.`, lineNumber: lineNumber });
                if (isSelfClosingSyntax && !this.selfClosingTags.has(tagName)) errors.push({ message: `<strong>Invalid Syntax:</strong> The &lt;${tagName}&gt; tag should not be self-closing.`, lineNumber: lineNumber });
                ...
                if ((/\bwidth\s*=/.test(attributesString) || /\bheight\s*=/.test(attributesString)) && !this.mediaSizingElements.has(tagName)) errors.push({ message: `<strong>Improper Attribute:</strong> 'width' or 'height' on &lt;${tagName}&gt;. Use CSS for sizing.`, lineNumber: lineNumber });
                if (/\btarget\s*=/.test(attributesString) && !this.targetValidElements.has(tagName)) errors.push({ message: `<strong>Improper Attribute:</strong> 'target' on &lt;${tagName}&gt;.`, lineNumber: lineNumber });
        ...
        tagStack.forEach(unclosedTag => {
            if (!unclosedTag.ignored) {
                errors.push({ message: `<strong>Unclosed Tag:</strong> The &lt;${unclosedTag.tagName}&gt; tag (from line ${unclosedTag.lineNumber}) was never closed.`, lineNumber: unclosedTag.lineNumber });
            }
        });
    },
	
	wet4AllowedClasses: new Set([
    // Accessibility & WET Core
    'hidden', 'show', 'wb-inv', 'wb-init', 'wb-invisible', 'simple', 'complex',

    // Alerts, Wells & Panels
    'alert', 'alert-success', 'alert-info', 'alert-warning', 'alert-danger', 'alert-branch',
    'panel', 'panel-default', 'panel-primary', 'panel-success', 'panel-info', 'panel-warning', 'panel-danger',
    'panel-heading', 'panel-title', 'panel-body', 'panel-footer', 'panel-group',
    'well', 'well-sm', 'well-lg',

    // Buttons
    'btn', 'btn-default', 'btn-primary', 'btn-success', 'btn-info', 'btn-warning', 'btn-danger', 'btn-link',
    'btn-lg', 'btn-sm', 'btn-xs', 'btn-block', 'btn-call-to-action',

    // Headings & Display
    'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'page-header',

    // Forms
    'form-control', 'form-group', 'control-label', 'help-block', 'checkbox', 'radio',
    'form-inline', 'form-horizontal', 'input-lg', 'input-sm', 'input-group', 'input-group-btn',
    ...
    'lst-spcd', 'lst-lwr-alph', 'lst-upr-alph', 'lst-lwr-rmn', 'lst-upr-rmn', 'lst-num',
    
    // Navigation & Pagination
    'nav', 'nav-tabs', 'nav-pills', 'nav-stacked', 'navbar', 'navbar-nav', 'navbar-default',
    'breadcrumb', 'pagination', 'pagination-lg', 'pagination-sm', 'pager', 'previous', 'next',

    // Tables
    'table', 'table-striped', 'table-bordered', 'table-hover', 'table-condensed', 'table-responsive',
    'active', 'success', 'info', 'warning', 'danger', 'nowrap', 'dataTable', 'wb-tables-inited',

    // WET Component Main Classes
    'wb-tabs', 'wb-details', 'wb-equalize', 'wb-overlay', 'wb-modal', 'wb-share', 'wb-feeds',
    'wb-filter', 'wb-frmvld', 'wb-geomap', 'wb-graph', 'wb-tables', 'wb-data-ajax',
    'wb-toggle', 'wb-menu', 'wb-fnote', 'wb-prettify', 'wb-fieldflow', 'wb-steps',
    'wb-chtwzrd', 'wb-multimedia', 'wb-zebra',
    
    // WET Component Inner/State Classes
    'tab-count', 'tgl-tab', 'wb-fltr-inited', 'frmvld-wtt',
    'fn-rtn', 'fn-lnk', 'mfp-hide',
    'modal-content', 'modal-header', 'modal-body', 'modal-footer', 'modal-open',
    'gc-active', 'gc-byline', 'gc-stp-stp', 'gc-rprt-prblm',

    // Pre-WET4 / Deprecated Classes (WET v3, etc.)
    ...
    'push_1', 'push_2', 'push_3', 'push_4', 'push_5', 'push_6', 'push_7', 'push_8', 'push_9', 'push_10', 'push_11', 'push_12', 'push_13', 'push_14', 'push_15',
    'pull_1', 'pull_2', 'pull_3', 'pull_4', 'pull_5', 'pull_6', 'pull_7', 'pull_8', 'pull_9', 'pull_10', 'pull_11', 'pull_12', 'pull_13', 'pull_14', 'pull_15',
    'alpha', 'omega', 'clear', 'show-grid',
    'wet-boew-lightbox', 'wet-boew-zebra', 'table-simplify', 'wet-boew-footnotes',
    'wet-boew-button', 'button-accent', 'button-attention',

    // --- UTILITY CLASSES (Bootstrap, Tailwind, etc.) ---

    // Layout, Display & Position
    'block', 'inline-block', 'inline', 'flex', 'inline-flex', 'grid', 'table-cell',
    'static', 'relative', 'absolute', 'fixed', 'sticky',
    'visible', 'invisible',
    'clearfix', 'center-block', 'pull-left', 'pull-right',
    'top-0', 'bottom-0', 'left-0', 'right-0',
    'z-0', 'z-10', 'z-20', 'z-30', 'z-40', 'z-50', 'z-auto',

    // Sizing (Width, Height)
    'w-0', 'w-1', 'w-2', 'w-3', 'w-4', 'w-5', 'w-6', 'w-8', 'w-10', 'w-12', 'w-16', 'w-20', 'w-24', 'w-32', 'w-40', 'w-48', 'w-56', 'w-64', 'w-auto', 'w-px', 'w-full', 'w-screen',
    'h-0', 'h-1', 'h-2', 'h-3', 'h-4', 'h-5', 'h-6', 'h-8', 'h-10', 'h-12', 'h-16', 'h-20', 'h-24', 'h-32', 'h-40', 'h-48', 'h-56', 'h-64', 'h-auto', 'h-px', 'h-full', 'h-screen',
    ...
    'text-uppercase', 'text-lowercase', 'text-capitalize', 'text-nowrap', 'text-hide', 'pre-scrollable',
    'text-xs', 'text-sm', 'text-base', 'text-lg', 'text-xl', 'text-2xl', 'text-3xl', 'text-4xl', 'text-5xl', 'text-6xl',
    'fnt-nrml', 'fnt-bld', 'fnt-ital', 'fnt-sz-sm', 'fnt-sz-md', 'fnt-sz-lg', 'fnt-sz-xl',

    // Colors (Text, Background, Border) - Common examples
    ...
    'text-red-500', 'text-yellow-500', 'text-green-500', 'text-blue-500', 'text-indigo-500', 'text-purple-500', 'text-pink-500',
    ...
    'space-x-2', 'space-y-2', // Common examples

    // Spacing (Margin & Padding) - WET Style
    'mrgn-tp-0', 'mrgn-tp-sm', 'mrgn-tp-md', 'mrgn-tp-lg', 'mrgn-tp-xl',
    'mrgn-bttm-0', 'mrgn-bttm-sm', 'mrgn-bttm-md', 'mrgn-bttm-lg', 'mrgn-bttm-xl',
    'mrgn-lft-0', 'mrgn-lft-sm', 'mrgn-lft-md', 'mrgn-lft-lg', 'mrgn-lft-xl',
    'mrgn-rght-0', 'mrgn-rght-sm', 'mrgn-rght-md', 'mrgn-rght-lg', 'mrgn-rght-xl',
    'pddng-tp-0', 'pddng-tp-sm', 'pddng-tp-md', 'pddng-tp-lg', 'pddng-tp-xl',
    'pddng-bttm-0', 'pddng-bttm-sm', 'pddng-bttm-md', 'pddng-bttm-lg', 'pddng-bttm-xl',
    'pddng-lft-0', 'pddng-lft-sm', 'pddng-lft-md', 'pddng-lft-lg', 'pddng-lft-xl',
    'pddng-rght-0', 'pddng-rght-sm', 'pddng-rght-md', 'pddng-rght-lg', 'pddng-rght-xl',

    // Spacing (Margin & Padding) - Tailwind/Bootstrap Style
    ...
    'brdr-tp', 'brdr-rght', 'brdr-bttm', 'brdr-lft', 'brdr-0',
...
richTextEditorTemplate: `<!DOCTYPE html>
    ...
    getColophonHtml: function({ language, monarch, year, identifier, number }) {
